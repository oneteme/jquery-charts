  ...
  funnelExample: {
    ...
    config: {
      ...
      options: {
        ...
        xaxis: {
          categories: ['Visites', 'Paniers', 'Checkout', 'Paiement', 'Livraison'],
          labels: {
            show: false
          }
        },
        ...
        tooltip: {
          ...
          custom: function({series, seriesIndex, dataPointIndex, w}) {
            const data = FUNNEL_CHART_DATA.funnelExample.data[dataPointIndex];
            let prevValue = dataPointIndex > 0 ? 
              FUNNEL_CHART_DATA.funnelExample.data[dataPointIndex-1].count : null;
            
            let conversionRate = prevValue ? 
              ((data.count / prevValue) * 100).toFixed(1) + '%' : '100%';
            
            const formattedCount = data.count.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ");
            
            return `<div style="padding: 10px; background: white; border-radius: 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.15);">
              <span style="font-weight: bold; color: ${w.globals.colors[dataPointIndex]};">
                ${data.field}
              </span>
              <div style="margin: 5px 0; font-size: 13px;">${data.description}</div>
