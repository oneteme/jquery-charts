...
@Component({
  ...
  selector: 'app-echarts-detail',
  template: `
    <div class="echarts-page">
      <div class="detail-nav" *ngIf="section">
        <a routerLink="/charts/echarts" class="back-link">← Tous les types</a>
        <span class="detail-title">{{ section.label }}</span>
        <div class="prev-next">
          <a *ngIf="prev" [routerLink]="['/charts/echarts', prev.id]" class="nav-btn">‹ {{ prev.label }}</a>
          <a *ngIf="next" [routerLink]="['/charts/echarts', next.id]" class="nav-btn">{{ next.label }} ›</a>
        </div>
      </div>
...
export class EChartsDetailComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.paramSub = this.route.paramMap.subscribe(params => {
      ...
      if (idx === -1) { this.router.navigate(['/charts/echarts']); return; }
      this.section      = SECTIONS[idx];
      this.prev         = idx > 0 ? SECTIONS[idx - 1] : null;
      this.next         = idx < SECTIONS.length - 1 ? SECTIONS[idx + 1] : null;
      const example     = (ECHARTS_EXAMPLES as any)[this.section.exampleKey];
